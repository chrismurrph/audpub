...
(deftest batch-longest-from-1-to-12
  (let [longest-path (metrics/batch-longest-path-hof example/connected-graph :1)]
    (is (= [:3 :4 :7 :8 :9 :10 :12] (longest-path :12)))))

(comment
  (run-tests)
  (longest-path example/connected-graph :1 :12)
  (eccentricity example/connected-graph :1)
  (radius example/connected-graph)
  (diameter example/connected-graph)
  )
