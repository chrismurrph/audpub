(ns reveal.e03-chess-server-popups
  (:require [vlaaad.reveal.ext :as rx]
            [au.com.seasoft.general.dev :as dev]))

;;
;; The ::boards action is registered but doesn't seem to be available. Try again another day...
;;
...
(rx/defaction ::boards [x]
  (when (::server (meta x))
    (constantly
      (do
        (dev/log-on "boards check has passed and observable view")
        {:fx/type rx/observable-view
         :ref     x
         :fn      (fn [state]
                    (dev/log-on "In the fn that will show the chess board")
                    {:fx/type      :scroll-pane
                     :fit-to-width true
                     :content
                                   {:fx/type :flow-pane
